...
%       reside. THIS IS ALTERNATE TESTING SETUP
% DATE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
close all
    ...
    %CONSTANTS
    h = 0.0005; %timestep
    time = 1.5; %max time the program is run for
    steps = ceil(time / h);
    toPlot = 10; %how often it will plot
    tol = 0.01; %tolerance for how large a velocity must be to be considered "moving"
    friction = 'off'; %'on' for friction, 'off' for no friction
    ...
    %BALLS
    balls(1) = ball (pos1, vel1, r);
    balls(2) = ball (pos2, vel2, r);
%     balls(3) = ball (pos3, vel3, r);
%     balls(4) = ball (pos4, vel4, r);
    
    %MAKING THE TABLE
    dimensions = [1.07 2.13];
    table = board (balls, dimensions);
